...
def show_production_trends(df, smoothing=True):
    ...
    ax.scatter(max_date, grouped.max(), color='green', s=50, label='Max')
    ax.scatter(min_date, grouped.min(), color='red', s=50, label='Min')
    ax.set_xlabel('Date')
    ax.set_ylabel('Bottles Produced')
    ax.set_title('📈 Production Trend', fontsize=12)
...
def show_defect_rate_trend(df, smoothing=True):
    ...
    ax.scatter(grouped.loc[max_idx, 'date'], grouped['defect_rate'].max(), color='green', s=50, label='Max')
    ax.scatter(grouped.loc[min_idx, 'date'], grouped['defect_rate'].min(), color='red', s=50, label='Min')
    ax.set_xlabel('Date')
    ax.set_ylabel('Defect Rate (%)')
    ax.set_title('📉 Defect Rate Trend', fontsize=12)
    ax.tick_params(axis='x', rotation=45)
    ax.legend(fontsize=8)
    st.pyplot(fig)
...
def show_downtime_trend(df, smoothing=True):
    ...
    ax.set_title('🕒 Downtime Trend', fontsize=12)
    ax.tick_params(axis='x', rotation=45)
    ax.legend(fontsize=8)
    st.pyplot(fig)
...
def show_defect_vs_production_scatter(df):
    ...
    ax.set_title('Defect vs Production Scatter', fontsize=11)
    st.pyplot(fig)
...
def show_shift_breakdown(df):
    ...
    ax.set_title('Defect % by Shift', fontsize=11)
...
def show_plant_comparison(df):
    ...
    ax.set_title('Plant-wise Total Production', fontsize=11)
    for i, v in enumerate(grouped['bottles_produced']):
        ax.text(i, v + 0.5, f"{int(v)}", ha='center', fontsize=8)
    st.pyplot(fig)
